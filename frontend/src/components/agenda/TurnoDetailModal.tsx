...
const ESTADO_CONFIG: Record<string, { label: string, color: string, bg: string, border: string, icon: any }> = {
    'PENDIENTE': { label: 'Pendiente', color: 'text-blue-700', bg: 'bg-blue-100', border: 'border-blue-200', icon: Clock },
    'EN_CURSO': { label: 'En Curso', color: 'text-amber-700', bg: 'bg-amber-100', border: 'border-amber-200', icon: ArrowRight },
    'FINALIZADO': { label: 'Finalizado', color: 'text-green-700', bg: 'bg-green-100', border: 'border-green-200', icon: CheckCircle2 },
    'CANCELADO': { label: 'Cancelado', color: 'text-red-700', bg: 'bg-red-100', border: 'border-red-200', icon: AlertTriangle },
    'NO_ASISTIO': { label: 'No Asistió', color: 'text-gray-700', bg: 'bg-gray-100', border: 'border-gray-200', icon: User },
};
...
export default function TurnoDetailModal({ isOpen, onClose, turno, onSuccess, onCheckout }: TurnoDetailModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="detail-modal-title">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col transform transition-all animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                {/* Header */}
                <div className="flex justify-between items-start p-6 border-b border-gray-100">
                    <div>
                        <h3 id="detail-modal-title" className="text-xl font-black text-gray-900 tracking-tight">Detalle del Turno</h3>
                    ...
                    <button onClick={onClose} className="text-gray-400 hover:text-gray-600 transition p-1" aria-label="Cerrar modal">
                        <X className="w-6 h-6" />
                    </button>
                </div>
                ...
                <div className="p-6 space-y-5 overflow-y-auto">
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-gray-50/50 rounded-2xl p-4 border border-gray-100 transition-colors hover:bg-white">
                                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Cliente</span>
                                <p className="font-bold text-gray-900 truncate">{turno.cliente?.nombre || turno.title?.split(' - ')[0] || '—'}</p>
                                {turno.cliente?.telefono && (
                                    <p className="text-xs text-gray-500 font-medium mt-1 truncate">{turno.cliente.telefono}</p>
                                )}
                            </div>
                            <div className="bg-gray-50/50 rounded-2xl p-4 border border-gray-100 transition-colors hover:bg-white">
                                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Servicio</span>
                                <p className="font-bold text-gray-900 truncate">{turno.servicio?.nombre || turno.title?.split(' - ')[1] || '—'}</p>
                                {turno.servicio?.precio && (
                                    <p className="text-xs font-black text-blue-600 mt-1">${turno.servicio.precio}</p>
                                )}
                            </div>
                        </div>

                        <div className="bg-gray-50/50 rounded-2xl p-4 border border-gray-100">
                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Fecha y Horario</span>
                            <p className="font-bold text-gray-900 capitalize">
                                {formatDate(turno.fechaHoraInicio || turno.start)}
                            </p>
                            <div className="flex items-center text-sm text-gray-600 mt-1.5 font-semibold">
                        ...
                        {turno.barbero?.user?.nombre && (
                            <div className="bg-gray-50/50 rounded-2xl p-4 border border-gray-100">
                                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Atendido por</span>
                                <div className="flex items-center">
                                    <div className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 text-[10px] font-black mr-2">
                                        {turno.barbero.user.nombre.charAt(0)}
                                    </div>
                                    <p className="font-bold text-gray-900 text-sm">{turno.barbero.user.nombre}</p>
                                </div>
                            </div>
                        )}
                    </div>
